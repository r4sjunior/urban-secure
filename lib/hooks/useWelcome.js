...
export function useWelcome() {
  ...
  const load = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const { ok, data: json, error: erroHttp } = await fetchJson(`/api/welcome?wallet=${encodeURIComponent(address)}`);
  ...
  const receber = useCallback(async () => {
    ...
    try {
      ...
      const { ok, data: json, error: erroHttp } = await fetchJson('/api/welcome', {
        method: 'POST',
      ...
      if (!ok) { const msg = json.error || erroHttp; setError(msg); await load(); return { ok: false, error: msg }; }
