...
export function useAlbum() {
  ...
  const load = useCallback(async (target) => {
    ...
    setIsLoading(true);
    try {
      const { ok, data: json, error: erroHttp } = await fetchJson(`/api/stickers?wallet=${encodeURIComponent(target)}`);
      if (!ok) throw new Error(json.error || erroHttp || 'Erro ao carregar o álbum.');
  ...
  const openPack = useCallback(async () => {
    ...
    try {
      ...
      const { ok, data: json, error: erroHttp } = await fetchJson('/api/stickers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ wallet: address, action: 'open', ...auth }),
      });
      if (!ok) { const msg = json.error || erroHttp; setError(msg); return { ok: false, error: msg }; }
