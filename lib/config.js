...
//
// O multiplicador subiu de 1.6 para 2.5 em 2026-07-25, quando o claim diário
// passou a ser ON-CHAIN. Antes o resgate era só uma assinatura grátis; agora
// é uma transação que o usuário paga, e o PRIMEIRO resgate ainda cria a conta
// de estado dele (rent de 0.00139896 SOL, medido na devnet).
//
// Com 1.6× a cadeia fechava por 0.00083 SOL:
//   0.00560 (boas-vindas) − 0.00337 (mint) − 0.00148 (rent + taxa) = 0.00083
// Qualquer variação de priority fee em rede congestionada deixaria o recém-
// chegado com a arte registrada e SEM conseguir resgatar — travado no pior
// ponto possível, logo depois de fazer tudo certo.
//
// Com 2.5× a sobra vai a ~0.0039 SOL, uma margem de ~4.7x sobre o custo do
// resgate. Ver ARCHITECTURE.md § 10.
export const WELCOME_CLAIM_SOL = Number((ART_MINT_COST_SOL * 2.5).toFixed(6));
...
//
// ATENÇÃO: existem DOIS tetos desde a migração on-chain, e eles são contados
// separadamente:
//   - este aqui, off-chain, cobre o claim de BOAS-VINDAS (lib/social/welcome
//     grava no FAUCET_LEDGER do Pinata);
//   - o `daily_budget` da conta de treasury do programa cobre o CLAIM DIÁRIO,
//     e é ajustado por `POST /api/admin/treasury {"action":"budget"}`.
// Mudar um não muda o outro. Mantenha os dois alinhados, senão o app recusa
// resgate por um limite que ninguém consegue ver aqui.
//
// Subiu de 2 para 5 em 2026-07-25 para acomodar uma rodada de testes com
// convidados: a 0.00875 por boas-vindas, 5 SOL comportam ~570 carteiras novas
// por dia — muito além do previsto, e é justamente essa folga que evita que
// um teste com amigos esbarre no teto e pareça um bug do app.
export const DAILY_TREASURY_BUDGET_SOL = envFloat('DAILY_TREASURY_BUDGET_SOL', 5);
