...
export async function payForLike(wallet, artistWallet) {
  ...
  // Sem WebSocket (Vercel não suporta) — confirmação via polling HTTP
  ...
  // Confirmação 100% via polling HTTP (sem WebSocket).
  // Verifica o status da assinatura repetidamente até confirmar ou expirar.
  for (let i = 0; i < 20; i++) {
