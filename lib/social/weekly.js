...
/**
 * O mesmo identificador em número, para o PDA do comprovante de prêmio.
 *
 * O programa recebe `week_id: u32` e usa `["payout", week_id, posicao]` como
 * seeds — a string ISO não serve como seed sem virar bytes de tamanho variável.
 * "2026-W31" vira 202631: ano × 100 + número da semana. Cabe em u32 até o ano
 * 42949672, é crescente no tempo e continua legível a olho nu num explorer,
 * que importa quando se está conferindo um pagamento à mão.
 *
 * @param {string} id identificador ISO, ex. "2026-W31"
 * @returns {number}
 */
export function weekIdToU32(id) {
  const m = /^(\d{4})-W(\d{2})$/.exec(String(id || ''));
  if (!m) throw new Error(`Identificador de semana inválido: ${id}`);
