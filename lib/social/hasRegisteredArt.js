...
export async function hasArtOnChain(wallet) {
  ...
  try {
    ...
    // direto ao claim é recusado com "registre sua primeira arte" — logo
    // depois de ter registrado, que é a hora mais frustrante possível.
    //
    // Só o primeiro claim de cada carteira passa por aqui, então o custo
    // desta espera não aparece no uso do dia a dia.
    for (let tentativa = 0; tentativa < 2; tentativa++) {
      ...
      // O Core não guarda `symbol` on-chain, só `name` e `uri`. Para saber se
      // o asset é do app é preciso ler o JSON — mas só até achar UM, porque a
